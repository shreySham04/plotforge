...
import { getCurrentUser, login as loginApi, loginWithGoogle as loginWithGoogleApi, register as registerApi, updateMe } from "../services/authService";
...
function userFromAuthResponse(response) {
  ...
  return {
    ...
    role: response.role || (isOwner ? "OWNER" : "WRITER"),
    isOwner,
    profileImage: response.profileImage || "",
    theme: response.theme || "dark",
    themePreset: response.themePreset || "default",
    colorAccent: response.colorAccent || "cyan",
    aiPersona: response.aiPersona || "jack_sparrow"
  };
}
  ...
  const value = useMemo(
    () => ({
      ...
      async loginWithGoogle(googleData) {
        const response = await loginWithGoogleApi(googleData);
        if (!response?.token) {
