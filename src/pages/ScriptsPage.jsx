...
export default function ScriptsPage() {
  ...
  async function loadScripts() {
    ...
    try {
      ...
      setProjects(list.filter((p) => p.type === "SCRIPT"));
    } catch (err) {
      setError(extractApiError(err, "Could not load scripts"));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setError(extractApiError(err, "Could not create screenplay project"));
    }
  }
  ...
  async function handleDelete(id) {
    try {
      ...
      setError(extractApiError(err, "Could not delete script"));
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#070b12] text-slate-900 dark:text-slate-100 flex flex-col transition-colors duration-200">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 flex-1 space-y-8">
        <header className="space-y-2">
          <div className="theme-pill">
            <span>🎬</span> Screenplay Workspace
          </div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Scene &amp; Screenplay Projects</h1>
          <p className="text-xs text-slate-600 dark:text-slate-400">
            Draft industry-formatted screenplays, scene action headings, character dialogues, and parentheticals.
          </p>
        </header>
          ...
          <div className="rounded-2xl theme-bg-subtle p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-slate-800 dark:text-slate-200">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-300/80">You can browse and read all community screenplays below. Sign up or log in to draft and format your own screenplays!</p>
              </div>
            </div>
        ...
        <section className="card space-y-4">
          <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Create New Screenplay</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Script Title</label>
              <input
                type="text"
                className="input text-sm"
                placeholder="e.g. Cyberpunk 2099: Pilot"
                value={title}
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/60 p-3 space-y-3">
              <label className="flex items-center gap-2.5 text-xs text-slate-700 dark:text-slate-300 cursor-pointer">
                ...
                />
                <span>This screenplay is connected to an existing movie/series</span>
              </label>
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-800/60 grid gap-3 sm:grid-cols-2 items-start">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <option value="ALTERNATE_ENDING">Alternate Timeline / Cut</option>
                    </select>
                  </div>
                </div>
              )}
        ...
        <section className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 p-3.5 rounded-2xl shadow-sm">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              className="input text-xs pl-9 pr-8 py-2 w-full dark:bg-slate-950/80 border-slate-300 dark:border-slate-700/80 focus:border-blue-500 transition rounded-xl"
              placeholder="Search screenplays by title, genre, subject, or tag..."
              value={searchQuery}
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {loading && safeArray(projects).length === 0 && (
          ...
          {!loading && safeArray(projects).length === 0 && (
            <div className="card col-span-full text-center text-xs text-slate-400 py-8">
              No screenplays found. Create your first screenplay above!
            </div>
          )}
          {safeArray(projects)
            .filter((p) => {
              if (selectedGenre !== "ALL") {
                ...
                if (!pGenres.includes(selectedGenre) && p.subject !== selectedGenre) return false;
              }
              if (searchQuery.trim()) {
      ...
      {(() => {
        ...
        return (
          <button
            ...
            title={`Consult ${persona.name} for Screenplay Advice`}
          >
      ...
      <AICopilotModal
        ...
        projectTitle="Screenplays Workspace"
      />

      <AuthModal
        isOpen={authModalOpen}
        onClose={() => setAuthModalOpen(false)}
        title="Sign In Required to Create Screenplays"
        description="You are browsing PlotForge as a guest. Please create an account or sign in to draft, format, and save screenplays."
      />
